...
export async function normalizeSelector(options = {}) {
  ...
  if (typeof selector === 'string' && engine === 'playwright' && isPlaywrightTextSelector(selector)) {
    ...
    try {
      ...
      if (isNavigationError(error)) {
        console.log('⚠️  Navigation detected during normalizeSelector (Playwright), returning null');
        return null;
      }
      throw error;
    }
  ...
  if (selector._isPuppeteerTextSelector) {
    try {
      ...
      if (isNavigationError(error)) {
        console.log('⚠️  Navigation detected during normalizeSelector (Puppeteer), returning null');
        return null;
      }
      throw error;
    }
